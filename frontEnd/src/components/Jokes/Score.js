...
export default function Score({ chiste, user }) {
  ...
  const fetchAverageScore = async () => {
    try {
      const response = await fetch(`${apiUrl}/jokes/${chiste._id}/average-score`);
      if (response.ok) {
  ...
  useEffect(() => {
    if (user && chiste.userScores) {
      const userScoreData = chiste.userScores.find((score) => score.email === user.email);
      if (userScoreData) {
      ...
      {hasVoted ? (
        <img
          ...
          className={`averageScore score-image ${
            averageScore >= 1 && averageScore <= 1.6 ? 'selected' :
            averageScore >= 1.61 && averageScore <= 2.5 ? 'selected' :
            averageScore >= 2.51 && averageScore <= 3.5 ? 'selected' :
            averageScore >= 3.51 && averageScore <= 4.2 ? 'selected' : 'selected'
          }`}
          src={
            averageScore >= 1 && averageScore <= 1.6 ? score1 :
            averageScore >= 1.61 && averageScore <= 2.5 ? score2 :
            averageScore >= 2.51 && averageScore <= 3.5 ? score3 :
            averageScore >= 3.51 && averageScore <= 4.2 ? score4 : score5
          }
        ...
        <div className="score-start">
          <img
            className={`score-image score-button ${userScore === 1 ? 'selected' : ''}`}
            src={score1}
            alt="Puntuación 1"
            onClick={() => handleScoreClick(1)}
          />
          <img
            className={`score-image score-button ${userScore === 2 ? 'selected' : ''}`}
            src={score2}
            alt="Puntuación 2"
            onClick={() => handleScoreClick(2)}
          />
          <img
            className={`score-image score-button ${userScore === 3 ? 'selected' : ''}`}
            src={score3}
            alt="Puntuación 3"
            onClick={() => handleScoreClick(3)}
          />
          <img
            className={`score-image score-button ${userScore === 4 ? 'selected' : ''}`}
            src={score4}
            alt="Puntuación 4"
            onClick={() => handleScoreClick(4)}
          />
          <img
            className={`score-image score-button ${userScore === 5 ? 'selected' : ''}`}
            src={score5}
            alt="Puntuación 5"
